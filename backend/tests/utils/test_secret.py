...
def test_debug_logging_file_success(temp_secret_file, caplog_debug):
    ...
    with patch.dict(os.environ, {
        ...
        assert f"Loaded secret 'TEST_SECRET' from file: {temp_secret_file}" in caplog_debug.text
...
def test_debug_logging_file_not_found(caplog_debug):
    ...
    with patch.dict(os.environ, {
        ...
        get_secret('TEST_SECRET')
        assert "File path specified for 'TEST_SECRET' but file not found" in caplog_debug.text
...
def test_debug_logging_env_var_success(caplog_debug):
    ...
    with patch.dict(os.environ, {
        ...
        get_secret('TEST_SECRET')
        assert "Loaded secret 'TEST_SECRET' from environment variable" in caplog_debug.text
...
def test_debug_logging_not_found(caplog_debug):
    ...
    with patch.dict(os.environ, {
        ...
        get_secret('TEST_SECRET')
        assert "Secret 'TEST_SECRET' not found in environment or file" in caplog_debug.text

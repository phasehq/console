...
from backend.api.utils.syncing.aws.secrets_manager import (
...
class TestSecretsManager(unittest.TestCase):
    ...
    @patch('backend.api.utils.syncing.aws.secrets_manager.get_secrets_client_from_session')
    @patch('backend.api.utils.syncing.aws.secrets_manager.get_aws_sts_session')
    def test_list_aws_secrets_with_role_arn_success(self, mock_get_aws_sts_session, mock_get_secrets_client_from_session):
        # Arrange
        ...
        page1_secrets = [{"Name": "secret1", "ARN": "arn1"}, {"Name": "secret2", "ARN": "arn2"}]
        ...
        # Assert
        mock_get_aws_sts_session.assert_called_once_with(mock_role_arn, mock_region, mock_external_id)
        mock_get_secrets_client_from_session.assert_called_once_with(mock_session_instance)
    ...
    @patch('backend.api.utils.syncing.aws.secrets_manager.get_client')
    def test_list_aws_secrets_with_access_keys_success(self, mock_get_client):
        ...
        mock_paginator.paginate.return_value = [
            {"SecretList": page_secrets}
        ]

        expected_secrets_list = [
            {"name": "secret_key_1", "arn": "arn_key_1"}
        ]
        ...
        # Assert
        mock_get_client.assert_called_once_with(mock_access_key_id, mock_secret_access_key, mock_region)
        mock_secrets_client.get_paginator.assert_called_once_with("list_secrets")
        mock_paginator.paginate.assert_called_once()
        self.assertEqual(secrets_list, expected_secrets_list)
    ...
    def test_list_aws_secrets_no_auth_provided(self):
        ...
        # Act & Assert
        with self.assertRaisesRegex(Exception, "Please provide either assume role parameters or access keys."):
    ...
    @patch('backend.api.utils.syncing.aws.secrets_manager.get_secrets_client_from_session')
    @patch('backend.api.utils.syncing.aws.secrets_manager.get_aws_sts_session')
    def test_sync_aws_secrets_with_role_arn_update_by_arn_success(self, mock_get_aws_sts_session, mock_get_secrets_client_from_session):
        # Arrange
        ...
        mock_arn = "arn:aws:secretsmanager:us-west-2:123456789012:secret:MySecret-xxxxxx"
        ...
        self.assertEqual(result, {"message": f"Secret '{mock_arn}' updated with provided secrets."})
        mock_get_aws_sts_session.assert_called_once_with(mock_role_arn, mock_region, mock_external_id)
        mock_get_secrets_client_from_session.assert_called_once_with(mock_session_instance)
    ...
    @patch('backend.api.utils.syncing.aws.secrets_manager.get_client')
    def test_sync_aws_secrets_with_access_keys_create_by_name_success(self, mock_get_client):
        # Arrange
        mock_region = "eu-central-1"
        mock_access_key_id = "AKIA2OGYBAH63UA3VNFG"
        mock_secret_access_key = "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"
        ...
        # Assert
        self.assertTrue(success)
        self.assertEqual(result, {"message": f"New secret '{mock_secret_name}' created."})
        mock_get_client.assert_called_once_with(mock_access_key_id, mock_secret_access_key, mock_region)
        mock_sm_client.get_secret_value.assert_called_once_with(SecretId=mock_secret_name)
    ...
    @patch('backend.api.utils.syncing.aws.secrets_manager.get_client')
    def test_sync_aws_secrets_with_access_keys_update_by_name_success(self, mock_get_client):
        # Arrange
        ...
        # Assert
        self.assertTrue(success)
        self.assertEqual(result, {"message": f"Existing secret '{mock_secret_name}' updated."})
        mock_get_client.assert_called_once_with(mock_access_key_id, mock_secret_access_key, mock_region)
        mock_sm_client.get_secret_value.assert_called_once_with(SecretId=mock_secret_name)
        mock_sm_client.update_secret.assert_called_once_with(
    ...
    def test_sync_aws_secrets_no_auth_provided(self):
        ...
        # Act
        success, result = sync_aws_secrets(mock_secrets, mock_region, mock_secret_name, None)

        # Assert
        self.assertFalse(success)
        self.assertEqual(result, {"message": "Please provide either assume role parameters or access keys."})
    ...
    def test_sync_aws_secrets_no_arn_or_name_provided(self):
        ...
        # Assert
        self.assertFalse(success)
        self.assertEqual(result, {"message": "Please provide either a secret ARN or a name."})
    ...
    def test_sync_aws_secrets_client_error_on_update(self, mock_get_client):
        ...
        error_response = {'Error': {'Code': 'InvalidRequestException', 'Message': 'Update failed'}}
        mock_sm_client.update_secret.side_effect = ClientError(error_response, 'update_secret')
        ...
        # Assert
        self.assertFalse(success)
        self.assertEqual(result, {"message": str(ClientError(error_response, 'update_secret'))})
        mock_sm_client.update_secret.assert_called_once()

    @patch('backend.api.utils.syncing.aws.secrets_manager.get_client') # Mock get_client
    @patch('backend.api.utils.syncing.aws.secrets_manager.json.dumps') # Mock json.dumps
    def test_sync_aws_secrets_unexpected_error(self, mock_json_dumps, mock_get_client):
        ...
        # Assert
        self.assertFalse(success)
        self.assertEqual(result, {"message": "An unexpected error occurred: Unexpected JSON processing error"})
        mock_get_client.assert_called_once_with(mock_access_key_id, mock_secret_access_key, mock_region)

...
def user_is_admin(user_id, org_id):
    ...
    member = OrganisationMember.objects.get(user_id=user_id, organisation_id=org_id)
...
def user_can_access_environment(user_id, env_id):
    ...
    org_member = OrganisationMember.objects.get(organisation=env.app.organisation, user_id=user_id)

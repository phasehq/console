...
class GenericOpenIDConnectAdapter(OAuth2Adapter):
    ...
    def complete_login(self, request, app, token, **kwargs):
        try:
            ...
            logger.debug(f"User authentication data received for email: {extra_data.get('email')}")
            ...
            try:
                send_login_email(request, extra_data['email'], extra_data.get('name', ''), self.get_provider().name)
            except Exception as e:
                logger.warning(f"Failed to send login email to {extra_data['email']}: {str(e)}")

...
export const DynamicSecret = ({ secret }: { secret: DynamicSecretType }) => {
  ...
  return (
    <div className="p-2 ring-1 ring-inset ring-neutral-400/20 flex flex-col gap-3 w-full rounded-lg group justify-between bg-zinc-100 dark:bg-zinc-800">
      <div>
        <div className="font-mono text-neutral-500 text-xs">{secret.path}</div>
        <div>
          <div className="text-sm text-zinc-900 dark:text-zinc-100 flex items-center gap-1 px-1">
            <ProviderIcon providerId={secret.provider} /> {secret.name}
          </div>
          <div className="text-2xs text-neutral-500 px-1">{secret.description}</div>
        </div>
      </div>

      <div className="relative group flex items-center justify-between w-full">
        <div className="flex h-full items-center justify-start gap-2 opacity-0 group-hover:opacity-100 transition ease">
          {/* <CreateLeaseDialog secret={secret} /> */}
          <ManageLeasesDialog secret={secret} />
        </div>
        <div className="flex h-full items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition ease">

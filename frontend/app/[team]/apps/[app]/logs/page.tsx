...
export default function AppLogs({ params }: { params: { team: string; app: string } }) {
  const DEFAULT_PAGE_SIZE = 25
  const loglistEndRef = useRef<HTMLTableCellElement>(null)
  const tableBodyRef = useRef<HTMLTableSectionElement>(null)
  const [getAppLogs, { data, loading }] = useLazyQuery(GetAppLogs)
  ...
  const fetchLogs = (start: number, end: number) => {
    getAppLogs({
      ...
      if (result.data?.logs.length) {
        setLogList(logList.concat(result.data.logs))
      }
   ...
   */
  useEffect(() => {
    if (data?.logsCount) setTotalCount(data.logsCount)
  }, [data])

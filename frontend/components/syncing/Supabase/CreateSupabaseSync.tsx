...
import { EnvironmentType, ProviderCredentialsType, SupabaseProjectType } from '@/apollo/graphql'
...
export const CreateSupabaseSync = (props: { appId: string; closeModal: () => void }) => {
  ...
  const [createSupabaseSync, { loading: creating }] = useMutation(CreateNewSupabaseSync)

  const [credential, setCredential] = useState<ProviderCredentialsType | null>(null)
  const [projects, setProjects] = useState<SupabaseProjectType[]>([])
  const [project, setProject] = useState<SupabaseProjectType | null>(null)
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    if (credential === null) {
      ...
      const { data: projectsData } = await getSupabaseProjects({
        variables: {
    ...
    <div className="p-4 space-y-6">
      <div>
        ...
        <div className="text-neutral-500 text-2xs">
          Sync an environment with Supabase Edge Function secrets.
        </div>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="relative">
                <Combobox as="div" value={project} onChange={setProject}>
                  {({ open }) => (
                    <>
                      ...
                      >
                        <Combobox.Options as={Fragment}>
                          <div className="bg-zinc-300 dark:bg-zinc-800 p-2 rounded-b-md shadow-2xl z-20 absolute max-h-80 overflow-y-auto w-full border border-t-none border-neutral-500/20 divide-y divide-neutral-500/20">
                            {filteredProjects.map((project: SupabaseProjectType) => (
